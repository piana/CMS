jQuery.fn.colourPicker = function (conf) {
    ...
    return this.each(function () {
        ...
        var icon    = jQuery('<a href="#" class="colorpicker-button"><i class="icon-tint "></i></a>').insertAfter(select);
        var input    = jQuery('<input type="text" id="' + select.attr('name') + '" name="' + select.attr('name') + '" value="' + select.val() + '" size="6" class="colorpicker-input"/>').insertAfter(select);
        var loc        = '';
