{assign comments $commentsDB->fetchAll('status=1','createTime DESC','*','20')}
{if count($comments)>0}
	<table class="table table-striped tablesort">
		<thead>
		<tr>
			<th>{L key = "admin.moderator.comments.tpl.commentsactive"}</th>
	  ...
	  {foreach $comments as $element}
		<tr>
			<td>
				<span data-toggle="tooltip" data-original-title="{$element->content|strip_tags}" data-placement="left">
...
{else}
	<div class="pad">{L key = "admin.moderator.comments.tpl.noresults"}</div>
{/if}